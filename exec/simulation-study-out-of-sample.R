...
n_trials = 100 # Number of times to perform the simulation study
block_size = 24 * 365 # The size of a block
num_cores = 25 # Number of cores for parallelisation
B = 50 # The number of bootstraps used in the two-step model for estimating s^*
...
res = parallel::mclapply(
  ...
  FUN = function(i) {
    ...
    #n_σ = sample.int(4, 1)
    n_σ = 4
    σ_coeffs = c(runif(1, .5, 2), rnorm(n_σ, 0, .2))
...
c(mean = mean(diff_bootstraps),
  quantile(diff_bootstraps, c(0, .025, .05, .075, .1, .15, .2, .25)))
...
percentages = res$inclusion %>%
  dplyr::group_by(name, model, n_σ) %>%
  dplyr::mutate(percentage = base::mean(included)) %>%
  dplyr::select(name, model, percentage, n_σ) %>%
  dplyr::slice(1)
